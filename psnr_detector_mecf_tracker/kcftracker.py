"""
This code has been taken from https://github.com/uoip/KCFpy.
It is an implementation of the paper by Henriques et al. (2014).

References:
	Henriques, J.F., Caseiro, R., Martins, P. and Batista, J., 2014.
	High-speed tracking with kernelized correlation filters.
	IEEE transactions on pattern analysis and machine intelligence, 37(3), pp.583-596.
"""
import numpy as np
import cv2
import fhog
...
# KCF tracker
class KCFTracker:
	def __init__(self, hog=False, fixed_window=True, multiscale=False, occlusion_threshold=0.3):
	...
	def createGaussianPeak(self, sizey, sizex):
		...
		output_sigma = np.sqrt(sizex*sizey) / self.padding * self.output_sigma_factor#此处应该多少有问题，未考虑固定窗口大小的问题
		# output_sigma = 1.09544
		# print(output_sigma)
	...
	def getFeatures(self, image, inithann, scale_adjust=1.0, bbox=None, isUse=False):
		...
		if(inithann):
			...
			if(self.template_size > 1):
				if(padded_w >= padded_h):#取最大的值为96固定大小
	...
	def update(self, image, bbox=None, isUse=False):
		if isUse:
			if(bbox[0]+bbox[2] <= 0):  
				bbox[0] = -bbox[2] + 1
			if(bbox[1]+bbox[3] <= 0):  
				bbox[1] = -bbox[2] + 1
			if(bbox[0] >= image.shape[1]-1):  
				bbox[0] = image.shape[1] - 2
			if(bbox[1] >= image.shape[0]-1):  
				bbox[1] = image.shape[0] - 2
		...
		if isUse:
			loc, peak_value = self.detect(self._tmpl, self.getFeatures(image, 0, 1.0, bbox, isUse))
		else:
		...
		if(self.scale_step != 1):
			...
			if isUse:
				new_loc1, new_peak_value1 = self.detect(self._tmpl, self.getFeatures(image, 0, 1.0/self.scale_step, bbox, isUse))
			else:
				new_loc1, new_peak_value1 = self.detect(self._tmpl, self.getFeatures(image, 0, 1.0/self.scale_step))
			
			# Test at a bigger _scale
			if isUse:
				new_loc2, new_peak_value2 = self.detect(self._tmpl, self.getFeatures(image, 0, self.scale_step, bbox, isUse))
			else:
		...
		if isUse:
			self._roi[0] = cx - bbox[2]/2.0 + loc[0]*self.cell_size*self._scale
			self._roi[1] = cy - bbox[3]/2.0 + loc[1]*self.cell_size*self._scale
		else:
